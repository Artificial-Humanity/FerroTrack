...
7. **FerroStep is being installed with this repo as a deployment target** (owner,
   ...
   - ✅✅ **DEPLOYMENT IS A PULL, NOT A PUSH (owner, 2026-09-07, reversing an order given
     ...
     - ✅ **Fault procedure (owner, 2026-09-07): on hitting a problem with how FerroStep
       handles workflow, check FerroStep for an update. If no suitable update exists, or it
       does not fix the problem, tell FerroStep's resident.** ⚠ Step one reaches outside
       this repo — see the workspace-wide binary note above.
